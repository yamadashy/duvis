//! Classification rule tables + the `explain_*` / `classify_*` lookups
//! that consume them. Tables are module-level consts so the rule set
//! and the human-readable explanation share a single source of truth.
